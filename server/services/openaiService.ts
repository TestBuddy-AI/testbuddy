...
import { ICodeLanguage, ITestFunction, IUnitTestFile } from "../types";
...
export async function feedbackUnitTestsPrompt(
  codeLanguage: ICodeLanguage,
  unitTestGenerated: IUnitTestFile,
  error: string
): Promise<CreateChatCompletionResponse> {
  const initial = {
  ...
  const functions = unitTestGenerated.functions.map((func) => ({
    role: ChatCompletionRequestMessageRoleEnum.User,
    content: func.code,
  }));

  const tests = unitTestGenerated.functions.map((func) => ({
    role: ChatCompletionRequestMessageRoleEnum.User,
