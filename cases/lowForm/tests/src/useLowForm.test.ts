...
async function validateForm(formValues = {}, schema) {
  const getErrorAtField = async (field, value) => {
    try {
  ...
  const errorMessages = await Promise.all(
    formKeys.map((field) => getErrorAtField(field, formValues[field]))
  );

  const isFormInvalid = errorMessages.some((error) => Boolean(error));

  const fieldErrors = formKeys.reduce((acc, curr, index) => {
